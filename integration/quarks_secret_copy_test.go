...
var _ = Describe("QuarksCopies", func() {
	...
	Context("the user wants copies of the user password secret", func() {
		...
		FIt("should copy into other namespaces if copy empty secret if found", func() {
			By("Checking the quarkssecret status")
			checkStatus()
		...
		FIt("should update the copies in other namespaces if copy empty secret is found", func() {
			By("Checking the quarkssecret status")
			checkStatus()
	...
	Context("qsec wants copies of the user password secret", func() {
		...
		FIt("should copy into other namespaces if copy quarks secret if found", func() {
			By("Checking the quarkssecret status")
			checkStatus()
		...
		FIt("should update the copies in other namespaces if copy quarks secret if found", func() {
			By("Updating the user password secret")
			passwordSecret.StringData["password"] = "supersecurepassword"
			_, tearDown, err := env.UpdateSecret(env.Namespace, *passwordSecret)
			Expect(err).NotTo(HaveOccurred())
