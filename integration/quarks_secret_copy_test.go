...
var _ = Describe("QuarksUserSecret", func() {
	...
	Context("the secret is generated by operator for qsec copies", func() {
		...
		It("should copy the rotated generated secret to the copy namespace if copy qsec is valid", func() {
			By("Checking the quarkssecret status")
			checkStatus()
	...
	Context("the secret is generated by operator for qsec copies", func() {
		BeforeEach(func() {
			qsecName = env.Namespace + "test.qsec"
			copyNamespace = fmt.Sprintf("%s-%s", env.Namespace, "copy")
		...
		It("should copy into other namespaces if copy secret if present", func() {
			By("Checking the quarkssecret status")
			checkStatus()
		...
		It("should copy the rotated generated secret to the copy namespace if kube sec is valid", func() {
			By("Checking the quarkssecret status")
			checkStatus()
	...
	Context("the user wants copies of the user password secret", func() {
		...
		It("should update the copies in other namespaces if empty secret is present", func() {
			By("Checking the quarkssecret status")
			checkStatus()
	...
	Context("the qsec wants copies of the user password secret", func() {
		var passwordSecret *v1.Secret
		...
		It("should copy into other namespaces if copy quarks secret if present", func() {
			By("Checking the quarkssecret status")
			checkStatus()
		...
		It("should update the copies in other namespaces if qsec is present", func() {
			By("Updating the user password secret")
			passwordSecret.StringData["password"] = "supersecurepassword"
			_, tearDown, err := env.UpdateSecret(env.Namespace, *passwordSecret)
			tearDowns = append(tearDowns, tearDown)
			...
			Eventually(func() bool {
				...
				return true
			}, 5*time.Second).Should(Equal(false))
			checkStatus()
